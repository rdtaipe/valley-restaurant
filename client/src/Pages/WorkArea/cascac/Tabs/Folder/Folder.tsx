...
export default class Folder<T = any> extends React.PureComponent<{
  ...
  public render() {
    ...
    const fullId = scope.length
      ? getFullId!(scope[scope.length - 1], scope.slice(0, -1))
      : "";
    return (
        ...
        {name && (
          <div
            className={prefix(
              ...
              "full",
              "file",
              this.isSelected(fullId) ? "selected" : ""
            )}
  ...
  private renderProperties() {
    ...
    return keys.map((key) => {
      ...
      if (
        children &&
        (isArray(children) ? children.length : isObject(children))
      ) {
        return (
          <Folder<T>
            key={fullId}
  ...
  private onDrag = (e: OnDrag) => {
    ...
    if (!datas.dragFirst) {
      ...
      if (
        clickedTarget &&
        selected.indexOf(clickedTarget.getAttribute("data-file-key")!) === -1
      ) {
    ...
    if (prevInfo && isTop) {
      ...
      target = folderElement.querySelector<HTMLElement>(
        `[data-file-key="${key}"]`
      )!;
    ...
    let distDepth = isTop
      ? 0
      : between(
          Math.round((distX > 0 ? distX * 0.2 : distX) / 10),
          depthRange[0],
          depthRange[1]
        );
    ...
    if (
      nextInfo &&
      !isTop &&
      selected.indexOf(nextInfo.fullId) > -1 &&
    ...
    if (isMoveChildren) {
      ...
      if (
        fileInfos.every((info) =>
          info.scope.every((v, i) => v === prevScope[i])
        )
      ) {
        distDepth = 0;
      } else if (
        fileInfos.every((info) =>
          info.scope.every((v, i) => v === parentScope[i])
        )
      ) {
        distDepth = 1;
      } else {
  ...
  private onDragEnd = (e: OnDragEnd) => {
    ...
    const objMap = datas.objMap;
    const selectedInfos: Array<FileInfo<T>> = (this.props.selected || []).map(
      (id) => objMap[id]
    );
  ...
  private flatChildren(
    ...
    keys.forEach((key) => {
      ...
      if (
        valueChildren &&
        (isArray(valueChildren)
          ? valueChildren.length
          : isObject(valueChildren))
      ) {
    ...
    [].slice
      .call(
        document.querySelectorAll<HTMLElement>(
          ".scena-top-pointer, .scena-bottom-pointer"
        )
      )
